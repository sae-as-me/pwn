...
## 软链接
`ln -s [exp] [/tmp/echo]`之后执行具有suid权限(rwsr--r--中的s)的程序以获取对应用户权限，参考[https://www.bilibili.com/video/BV1uM4y1Q7MY/?spm_id_from=pageDriver&vd_source=c525ce304f4270daa217d87f25619231](https://www.bilibili.com/video/BV1uM4y1Q7MY/?spm_id_from=pageDriver&vd_source=c525ce304f4270daa217d87f25619231)
```c
//exp.c
int main()
{
...
：所以在拼接`system('/bin/sh')`时，应当寻找rdi（64位）或edi（32位）寄存器并先将`'/bin/sh'`读取后+`_system`的地址，
即`p64(rdi_ret_addr)+p64(binsh_addr)+p64(_system_addr)`
